...
//println();
  ...
  InstrumentConfig tekkin = new InstrumentConfig();

    tekkin.out = out;
    tekkin.waves = new String[] { "SINE","TRIANGLE","SINE" };

    // melody[i] の音階名を周波数に変換して、この音の基音にする
    tekkin.baseFreq = Frequency.ofPitch(noteName).asHz();
    

    tekkin.harmonics = new float[] {1.0,0.75,0.45,0.18,0.05};
    tekkin.cutoff = 4000.0;
    tekkin.res = 0.22;
    tekkin.filterMode = 0;
    tekkin.fcoRate = 0.0;
    tekkin.fcoAmount = 0.0;
